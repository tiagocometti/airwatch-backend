...
public class MeasurementsController(MeasurementService measurementService) : ControllerBase
{
    ...
    /// <summary>
    /// Retorna as medições mais recentes de todos os sensores.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    /// <param name="limit">Quantidade máxima de medições a retornar. Padrão: 50.</param>
    /// <returns>Lista das medições mais recentes.</returns>
    /// <response code="200">Lista retornada com sucesso. Pode ser uma lista vazia se não houver medições.</response>
    [HttpGet("latest")]
    [ProducesResponseType(typeof(IEnumerable<MeasurementDto>), StatusCodes.Status200OK)]
    ...
    /// <summary>
    /// Retorna o histórico de medições de um sensor específico.
    /// </summary>
    /// <remarks>
    ...
    /// <param name="limit">Quantidade máxima de medições a retornar. Padrão: 100.</param>
    /// <returns>Lista de medições do sensor informado.</returns>
    ...
    /// <summary>
    /// Retorna as medições de todos os sensores dentro de um período de tempo.
    /// </summary>
    /// <remarks>
    /// Útil para gerar gráficos históricos no frontend com base em um intervalo de datas selecionado pelo usuário.
    ///
    /// Exemplo: <c>GET /api/measurements/period?from=2026-03-01T00:00:00Z&amp;to=2026-03-12T23:59:59Z</c>
    /// </remarks>
    /// <param name="from">Data e hora inicial do período (UTC).</param>
    /// <param name="to">Data e hora final do período (UTC).</param>
    /// <returns>Lista de medições dentro do período informado.</returns>
    /// <response code="200">Lista retornada com sucesso. Pode ser vazia se não houver medições no período.</response>
